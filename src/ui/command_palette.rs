...
fn shortcuts_hint() -> Element<'static, Message> {
    row![
        text("↑↓").size(FONT_BADGE).color(TEXT_MUTED()).font(iced::Font::MONOSPACE),
        text("navigate").size(FONT_BADGE).color(TEXT_MUTED()),
        text("Enter").size(FONT_BADGE).color(TEXT_MUTED()).font(iced::Font::MONOSPACE),
        text("run").size(FONT_BADGE).color(TEXT_MUTED()),
    ]
    .spacing(6)
...
pub fn view(app: &ChatApp) -> Element<'_, Message> {
    ...
    for (index, cmd) in commands.iter().enumerate() {
        ...
        let mut cmd_row = row![
            text(cmd.label).size(FONT_SMALL).color(TEXT_HEAD()),
            iced::widget::Space::new().width(8),
            text(cmd.description).size(FONT_MICRO).color(TEXT_MUTED()),
            iced::widget::Space::new().width(Length::Fill),
        ].spacing(4).align_y(Alignment::Center);

        if !cmd.shortcut.is_empty() {
            cmd_row = cmd_row.push(
                text(cmd.shortcut.clone()).size(FONT_BADGE).color(TEXT_MUTED()).font(iced::Font::MONOSPACE)
            );
        }
    ...
    let modal = container(
        column![
            row![
                text("Command Palette").size(FONT_CAPTION).color(TEXT_MUTED()),
                iced::widget::Space::new().width(Length::Fill),
                shortcuts_hint(),
            ].align_y(Alignment::Center),
            input,
